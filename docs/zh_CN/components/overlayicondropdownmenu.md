...
将 `OverlayIconDropdownMenu` 放在 `TopAppBar`（或 `SmallTopAppBar`）的 `actions` 槽中，点击图标即可展开弹出框。这是最典型的使用场景——工具栏上的一个图标按钮，展开后呈现一组菜单项。
